...
async function startBulkFeed(payload, tabId) {
    ...
    try {
        ...
        for (let i = 0; i < total; i++) {
            ...
            broadcastBulkFeedProgress(tabId);
            logToServer(`[feed] Mengunggah feed (${i + 1}/${total})...`, 'info', tabId);
            ...
            try {
                ...
                broadcastBulkFeedProgress(tabId);
                logToServer(`[feed] Berhasil diposting (${i + 1}/${total})! ✨`, 'ok', tabId);
                ...
                logToServer(`[feed] Gagal posting (${imgFile}): ${e.message}`, 'err', tabId);
            ...
            if (i < total - 1 && bulkFeedState.running) {
                ...
                broadcastBulkFeedProgress(tabId);
                logToServer(`[feed] Menunggu jeda delay ${waitSecs}s...`, 'wait', tabId);
                await new Promise(r => setTimeout(r, delayMs));
            }
        }
        bulkFeedState.currentAction = 'All Done';
        logToServer(`[feed] Semua postingan selesai! 🎉`, 'ok', tabId);
    } catch (e) {
        bulkFeedState.currentAction = `Error: ${e.message}`;
