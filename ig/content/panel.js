...
(function () {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === 'SCRAPE_PROGRESS') {
      ...
      if (payload && (payload.type === 'task-update' || (payload.message && payload.message.startsWith('[@TASK_UPDATE@]')))) {
        ...
        // Exclude runner tools that already have their own dedicated UI boxes (DM, Like, Comment, View Story, Scraper)
        // and noisy background tech logs
        const isExcluded = /\[DM-.*\]|\[dm\]|broadcastPhoto|broadcastText|\[like\]|\[LIKE-RUNNER\]|media\.like|media\.likeComment|view[-_]?story|Watching stories|Found \d+ stories|Menemukan \d+ akun|\[comment\]|\[Shortlink\]|\[SKIP\]|\[vid-unique\]|CONFIG PROCESSED|can't project the same paths|Istirahat sejenak|Local Engine Berhasil Dimuat|Offline Mode/i.test(payload.message);

        if (!isExcluded) {
          appendSysLogEntry(payload.message, payload.type);
        }
      }
  ...
  $('btn-post-story').addEventListener('click', async () => {
    ...
    $('st-prog').style.display = 'block'; $('st-bar').style.width = '30%';
    $('st-status').textContent = 'Mengambil media. 10%';
      ...
      try {
        ...
        if (data.ok && data.logs && data.logs.length > 0) {
          const storyLogs = data.logs.filter(l => {
            const m = l.message;
            // Filter ketat: Hanya tampilkan tahap ramah pengguna, sembunyikan JSON/Technical
            const isFriendly = m.includes('[P') || m.includes('Phase') || m.includes('Tunggu sebentar') || m.includes('Mengunggah') || m.includes('Mempersiapkan') || m.includes('Tahap') || m.includes('Memproses') || m.includes('Mengonversi') || m.includes('Konversi') || m.includes('Menyiapkan') || m.includes('chunks') || m.includes('indexing') || m.includes('uploadFinish');
          ...
          if (storyLogs.length > 0) {
            ...
            let stageNum = 0;
            if (latest.includes('[P0]') || latest.includes('Memproses') || latest.includes('Mempersiapkan') || latest.includes('Mengonversi') || latest.includes('Konversi') || latest.includes('Menyiapkan')) {
              stageNum = 1;
            } else if (latest.includes('[P1]') || latest.includes('[P2]') || latest.includes('[P3]') || latest.includes('Mengunggah video') || latest.includes('Mengunggah cover') || latest.includes('chunks')) {
              stageNum = 2;
            } else if (latest.includes('[P4]') || latest.includes('[P5]') || latest.includes('Menunggu') || latest.includes('indexing') || latest.includes('uploadFinish')) {
              stageNum = 3;
            } else if (latest.includes('[P6]') || latest.includes('Configuring') || latest.includes('Finalisasi') || latest.includes('Finalizing') || latest.includes('selesai')) {
              stageNum = 4;
            }
            ...
            // Map teks status berdasarkan tahap maksimum yang telah dicapai
            if (maxStageReached === 1) {
              latest = 'Tahap 1: Mempersiapkan media (resize/blur/sticker). 35%';
            } else if (maxStageReached === 2) {
              latest = 'Tahap 2: Mengunggah data ke server Instagram. 55%';
            } else if (maxStageReached === 3) {
              latest = 'Tahap 3: Menunggu konfirmasi server. 80%';
            } else if (maxStageReached === 4) {
              latest = 'Tahap 4: Finalisasi posting. 95%';
            } else {
              latest = 'Mengambil media. 10%';
            }

            $('st-status').textContent = latest;

            // Update progress bar berdasarkan tahap maksimum
            if (maxStageReached === 1) $('st-bar').style.width = '35%';
            else if (maxStageReached === 2) $('st-bar').style.width = '55%';
            else if (maxStageReached === 3) $('st-bar').style.width = '80%';
            else if (maxStageReached === 4) $('st-bar').style.width = '95%';
            else if (latest.includes('Retrying')) $('st-bar').style.width = '70%';
    ...
    try {
      ...
      $('st-status').textContent = '✅ Story berhasil dipost! 100%';
