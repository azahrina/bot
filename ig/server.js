...
process.on('uncaughtException', (err) => {
  ...
  _originalError(msg);
  addLog('err', msg);
  try { fs.appendFileSync(path.join(process.cwd(), 'crash.log'), `[${new Date().toISOString()}] ${msg}\n\n`); } catch (e) { }
});
...
// Pengecekan Running Text berkala ke Cloud secara non-blocking (setiap 2 menit)
setInterval(async () => {
  try {
    ...
    const resp = await axios.post(AUTH_SERVER, {
      licenseKey: key,
      hwid: HWID,
      toolName: 'index'
    }, { timeout: 5000 });
    ...
    if (resp.data.ok) {
      ...
      if (resp.data.customer_name && resp.data.customer_name !== customerName) {
        customerName = resp.data.customer_name;
      }
      if (resp.data.running_text !== undefined && resp.data.running_text !== runningText) {
        runningText = resp.data.running_text;
        textChanged = true;
      }

      // Jika teks berjalan berubah, siarkan secara instan ke Chrome Extension via Socket.io!
      if (textChanged && typeof io !== 'undefined' && io) {
        io.emit('RUNNING_TEXT_UPDATE', { runningText });
      }
    }
  } catch (e) {
    // Gagal koneksi/offline diabaikan agar tidak mengganggu log
  }
}, 120000);
...
const SETUP_DIR = path.join(process.cwd(), 'setup');
const CONFIGS_DIR = path.join(process.cwd(), 'autosetup-configs');
const DM_DIR = path.join(process.cwd(), 'dm');
const FONTS_DIR = path.join(process.cwd(), 'fonts');
const AUTO_PROGRESS_DIR = path.join(process.cwd(), 'auto_progress');
const SESSION_DIR = path.join(process.cwd(), 'sessions');

// Metadata for tools: CLI menu removed (obsolete, replaced by chrome extension UI)

// --- API Endpoints ---

// Get the structured menu
// ---- License Gatekeeper API ---------------------------
app.get('/api/extension/license/check', async (req, res) => {
  if (!SvcHandler) await initProcess();
  const licensePath = path.join(process.cwd(), 'license.txt');
  const licenseKey = fs.existsSync(licensePath) ? (fs.readFileSync(licensePath, 'utf8').trim() || 'LOCAL_LIFETIME') : 'LOCAL_LIFETIME';
  res.json({ ok: true, license: licenseKey, customerName: 'Dibuat oleh mas abdul haris hamammi' });
});

app.post('/api/extension/license/activate', async (req, res) => {
  const { licenseKey } = req.body;
  if (licenseKey) {
    try { fs.writeFileSync(path.join(process.cwd(), 'license.txt'), licenseKey.trim()); } catch (e) { }
  }
  if (!SvcHandler) await initProcess();
...
app.get('/api/extension/shortlink-urls', (req, res) => {
  const filePath = path.join(process.cwd(), 'shortlink_urls.json');
  if (fs.existsSync(filePath)) {
...
// ---- Auto Setup Config -----------------------------------
const AUTO_SETUP_FILE = path.join(process.cwd(), 'autosetup_config.json');
...
app.post('/api/extension/autosetup/start', async (req, res) => {
  ...
  // Fallback to remote license verification only if not integrated
  addLog('info', `[LICENSE] Verifying license for autosetup...`, username);
  try {
    await syncSysData('autosetup');
    return res.status(500).json({ ok: false, error: 'Tool autosetup ditemukan di Cloud tapi logic eksekusinya belum diimplementasi di server.js.' });
  } catch (err) {
    addLog('err', err.message, username);
...
app.post('/api/extension/task/start', async (req, res) => {
  try {
    ...
    // --- FALLBACK: OLD SPAWN METHOD (For non-integrated tools) ---
    addLog('info', `[LICENSE] Verifying license for ${toolName}...`, username);

    let toolCode;
    try {
      toolCode = await syncSysData(toolName);
    } catch (err) {
      addLog('err', err.message, username);
      return res.status(403).json({ ok: false, error: err.message });
    }
    ...
    const processEnv = {
      ...
      DASHBOARD_MODE: 'true',
      EXTENSION_MODE: 'true',
      IG_SESSION_JSON: JSON.stringify(session || {}),
      ...env
    };

    let combinedCode;
    // Auto-detect if tool is already bundled (standalone)
    const isStandalone = toolCode.length > 50000 || toolCode.includes('class instagram');

    if (isStandalone) {
      addLog('info', `[SYSTEM] Tool ${toolName} detected as standalone. Neutralizing declarations and executing...`, username);

      // Engine-Fusion Strategy: Strip everything before the 'run' function and fuse it with the engine scope
      let fusedCode = toolCode;
      const runStartIndex = toolCode.search(/\basync\s+function\s+run\b|\bfunction\s+run\b/);

      if (runStartIndex !== -1) {
        addLog('info', `[SYSTEM] Engine-Fusion: Stripping redundant headers and fusing tool logic...`, username);
        fusedCode = toolCode.slice(runStartIndex);
      } else {
        addLog('warn', `[SYSTEM] Entry point 'run()' not found. Executing as-is.`, username);
      }

      combinedCode = "(function() {\n" + fusedCode + "\n})();";
    } else {
      const coreBase64 = Buffer.from(coreCodeCache).toString('base64');
      combinedCode =
        "const __coreCode = Buffer.from(" + JSON.stringify(coreBase64) + ", 'base64').toString();\n" +
        "const __origReq = require;\n" +
        "const __coreModule = { exports: {} };\n" +
        "require = function(p) {\n" +
        "  if (p === './index' || p === '../index' || p === './tools/index') {\n" +
        "    if (!__coreModule._loaded) {\n" +
        "      (new Function('module', 'exports', '__dirname', __coreCode))(__coreModule, __coreModule.exports, require('path').join(process.cwd(), 'tools'));\n" +
        "      __coreModule._loaded = true;\n" +
        "      global.instagram = __coreModule.exports.instagram;\n" +
        "      global.delay = __coreModule.exports.delay;\n" +
        "      global.reportTask = __coreModule.exports.reportTask;\n" +
        "      global.print = __coreModule.exports.print;\n" +
        "      global._ = __coreModule.exports._;\n" +
        "    }\n" +
        "    return __coreModule.exports;\n" +
        "  }\n" +
        "  return __origReq.apply(this, arguments);\n" +
        "};\n\n" +
        "(function() {\n" + toolCode + "\n})();";
    }

    const tmpFile = path.join(DATA_DIR, `task_${toolName}_${Date.now()}.js`);
    try {
      fs.writeFileSync(tmpFile, combinedCode);
    } catch (err) {
      addLog('err', `Gagal membuat file tugas: ${err.message}`, username);
      return res.status(500).json({ ok: false, error: 'Server error: failed to create task file' });
    }
    ...
    runningTasks.set(taskKey, child);
    ...
    child.stdout.on('data', (data) => {
      ...
      lines.forEach(line => {
        ...
        if (!trimmed) return;
        const match = trimmed.match(/\[@TASK_UPDATE@\](.*)/);
        if (match) {
          try {
            const taskData = JSON.parse(match[1].trim());
            const sseMsg = JSON.stringify({ type: 'task-update', data: taskData });
            const targets = [username, '__global__'];
            targets.forEach(u => {
              const clients = getSseClients(u);
              clients.forEach(client => { try { client.write(`data: ${sseMsg}\n\n`); } catch (e) { } });
            });
          } catch (e) { }
        } else {
          addLog('info', `[${toolName}] ${trimmed}`, username);
        }
      });
    });
    ...
    child.stderr.on('data', (data) => {
      ...
      if (msg) addLog('err', `[${toolName}] ${msg}`, username);
    });

    child.on('close', (code) => {
      runningTasks.delete(taskKey);
      addLog('info', `[${toolName}] Task exited with code ${code}`, username);
      // try { fs.unlinkSync(tmpFile); } catch (e) { } // Keeping for debugging
      // Notify UI
      const sseMsg = JSON.stringify({ type: 'task-finished', tool, code });
      const targets = [username, '__global__'];
      targets.forEach(u => {
        const clients = getSseClients(u);
        clients.forEach(client => { try { client.write(`data: ${sseMsg}\n\n`); } catch (e) { } });
    ...
    child.unref();
    res.json({ ok: true, message: `Task ${tool} started` });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
async function getExtClient(cookies, ua = null) {
  ...
  let client;
  if (extSessions.has(sessionid)) {
    client = extSessions.get(sessionid).client;
  } else {
    client = new SvcHandler();
    extSessions.set(sessionid, { client, ua });
  }

  const cookieArray = Array.isArray(cookies) ? cookies : parseCookiesToArray(cookies);
  await client.loginWithExtensionSession(true, false, { _cookies: cookieArray, ua: ua });
  return client;
}
...
app.get('/api/extension/license', (req, res) => {
  const licensePath = path.join(process.cwd(), 'license.txt');
  const key = fs.existsSync(licensePath) ? (fs.readFileSync(licensePath, 'utf8').trim() || 'LOCAL_LIFETIME') : 'LOCAL_LIFETIME';
  res.json({ ok: true, hwid: HWID, licenseKey: key, engineReady: !!SvcHandler, customerName: 'Dibuat oleh mas abdul haris hamammi', runningText: 'Offline Mode Active' });
});

app.post('/api/extension/license/save', async (req, res) => {
  try {
    const { licenseKey } = req.body;
    fs.writeFileSync(path.join(process.cwd(), 'license.txt'), (licenseKey || 'LOCAL_LIFETIME').trim());
    await initProcess();
...
app.get('/api/extension/setup-files/comment', (req, res) => {
  try {
    const dir = path.join(process.cwd(), 'setup/comment');
...
app.get('/api/extension/setup-file/comment', (req, res) => {
  try {
    const { filename } = req.query;
    const filePath = path.join(process.cwd(), 'setup/comment', path.basename(filename));
...
app.get('/api/extension/feed/bulk-info', (req, res) => {
  ...
  const captionFile = req.query.captionFile || 'media/caption.txt';
